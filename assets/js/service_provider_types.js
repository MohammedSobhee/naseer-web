$(document).ready(function () {
    ...
    if ($("#service_provider_types_tbl").length) {

        var service_provider_types_tbl = $("#service_provider_types_tbl");
        service_provider_types_tbl.on('preXhr.dt', function (e, settings, data) {
            ...
            "ajax": {
                url: baseURL + "/constants/service-provider-types-data"
                , "dataSrc": function (json) {
                    //Make your callback here.
                    if (json.status != undefined && !json.status) {
                        $('#service_provider_types_tbl_processing').hide();
        ...
        $(".dataTables_filter input")
            ...
            .bind("input", function (e) { // Bind our desired behavior
                ...
                if (this.value.length >= 3 || e.keyCode == 13) {
                    ...
                    service_provider_types_tbl.api().search(this.value).ajax.reload();

                }
                // Ensure we clear the search if they backspace far enough
                if (this.value == "") {
                    service_provider_types_tbl.api().search("").ajax.reload();
                }
            });
    }

    $(document).on('click', '.edit-service-provider-type-mdl', function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#edit-service-provider-type').modal('show', {backdrop: 'static', keyboard: false});
            }
        });

    });
    $(document).on('click', '.add-service-provider-type-mdl', function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#results-modals').html(data);
                $('#add-service-provider-type').modal('show', {backdrop: 'static', keyboard: false});
            }
        });
...
//                if ($(this).val().length > 3)
        service_provider_types_tbl.api().ajax.reload();
    });
