$(document).ready(function () {
    ...
    if ($("#constants_tbl").length) {
        ...
        constants_tbl.on('preXhr.dt', function (e, settings, data) {
            ...
            "ajax": {
                url: baseURL + "/constant/types-data"
    ...
    $(document).on('click', '#save', function (event) {
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: {'_token': csrf_token, 'constant_name': constant_name, 'category_id': category_id},
    ...
    $(document).on('click', '.edit', function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#edit-type').modal('show', {backdrop: 'static', keyboard: false});
                // if (data.status) {
                //     $('.constant_name').val(data.items.name);
                //     $('#category_id').val(data.items.category_id);
                //     $('#save_type_frm').attr('action', action);
                //
                // } else {
                //     toastr['error'](data.message);
                // }
...
//                if ($(this).val().length > 3)
    ...
    $(document).on('submit', '#save_type_frm,#formEdit', function (event) {
