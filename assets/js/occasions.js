var occasions_tbl = $("#occasions_tbl");
$(document).ready(function () {


    if ($("#occasions_tbl").length) {

        occasions_tbl.on('preXhr.dt', function (e, settings, data) {
            // data.talent_name = $('#talent_name').val();
        }).dataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                url: baseURL + "/occasion-data",
                "dataSrc": function (json) {
                    //Make your callback here.
                    if (json.status !== undefined && !json.status) {
                        $('#occasions_tbl_processing').hide();
                        location.reload();
                        //
                    } else
                        return json.data;
    ...
    $(document).on('click', '.add-occasions-mdl', function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#add-occasion').modal('show', {backdrop: 'static', keyboard: false});

            }
        });

    });
    $(document).on('click', '.edit-occasions-mdl', function (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#results-modals').html(data);
                $('#edit-occasion').modal('show', {backdrop: 'static', keyboard: false});
